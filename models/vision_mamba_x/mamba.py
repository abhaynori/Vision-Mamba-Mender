...
from models.vision_mamba_x.mamba_ssm.modules.mamba_simple import Mamba
from models.vision_mamba_x.mamba_ssm.utils.generation import GenerationMixin
from models.vision_mamba_x.mamba_ssm.utils.hf import load_config_hf, load_state_dict_hf

from models.vision_mamba_x.rope import *
import random

try:
    from models.vision_mamba.mamba_ssm.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if not self.fused_add_norm:
            ...
            if residual is None:  # first is true
                ...
                hidden_states, residual = fused_add_norm_fn(
                    # self.drop_path(hidden_states), #TODO drop path
                    hidden_states,
                    self.norm.weight,
                    self.norm.bias,
                    residual=residual,
        ...
        # hidden_states.register_hook(self.save_gradients)
...
def create_block(
    ...
    mixer_cls = partial(Mamba, layer_idx=layer_idx, bimamba_type=bimamba_type, if_devide_out=if_devide_out,
                        init_layer_scale=init_layer_scale, if_constraint=if_constraint, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class VisionMamba(nn.Module):
    def __init__(self,
                 ...
                 ssm_cfg=None,
                 drop_rate=0.,
                 drop_path_rate=0.0,  # TODO turn into original 0.1
        ...
        if constraint_layers is None:
            ...
            self.constraint_layers = [layer for layer in range(depth)]  # [all]
        else:
        ...
        self.layers = nn.ModuleList(
            [
                create_block(
                    ...
                    if_constraint=i in self.constraint_layers,
                    **factory_kwargs,
                )
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        residual = None
        hidden_states = x
        if not self.if_bidirectional:  # not false
...
@register_model
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2_x(pretrained=False, **kwargs):
    model = VisionMamba(
...
@register_model
def vim_tiny_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2_x(pretrained=False,
                                                                                               **kwargs):
    model = VisionMamba(
...
@register_model
def vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2_x(pretrained=False, **kwargs):
    model = VisionMamba(
...
@register_model
def vim_small_patch16_stride8_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2_x(pretrained=False,
                                                                                                **kwargs):
    model = VisionMamba(
...
@register_model
def vim_cifar_x(pretrained=False, **kwargs):
    model = VisionMamba(
        img_size=32, patch_size=4, stride=4, embed_dim=192, depth=8, rms_norm=True, residual_in_fp32=True,
        fused_add_norm=True,
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
...
@register_model
def vim_tiny_d12_x(num_classes=1000, pretrained=False, constraint_layers=None, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, num_classes=num_classes,
        constraint_layers=constraint_layers, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
