...
from models.local_mamba_x.mamba.multi_mamba import MultiMamba

from models.local_mamba_x.mamba.rope import *

from models.local_mamba_x.mamba_ssm.ops.triton.layernorm import RMSNorm, layer_norm_fn, rms_norm_fn
...
def create_block(
    ...
    mixer_cls = partial(mamba_cls, layer_idx=layer_idx, bimamba_type=bimamba_type, directions=directions,
                        token_size=token_size, use_middle_cls_token=use_middle_cls_token, if_constraint=if_constraint,
...
class VisionMamba(nn.Module):
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=channels, embed_dim=embed_dim, strict_img_size=False,
    ...
    def forward_features(self, x, inference_params=None, out_indices=None):
        ...
        if self.if_abs_pos_embed:
            ...
            if H != self.token_size[0] or W != self.token_size[1]:
                ...
                pos_embed = Backbone_LocalVisionMamba.resize_pos_embed(self.pos_embed, (H, W), self.token_size,
                                                                       'bicubic')
                if self.if_rope:
                    freqs_cos = Backbone_LocalVisionMamba.resize_pos_embed(self.rope.freqs_cos.unsqueeze(0), (H, W),
                                                                           self.token_size, 'bicubic')[0]
                    freqs_sin = Backbone_LocalVisionMamba.resize_pos_embed(self.rope.freqs_sin.unsqueeze(0), (H, W),
                                                                           self.token_size, 'bicubic')[0]
            else:
...
def local_vim_tiny_search(pretrained=False, **kwargs):
    directions = None
    model = VisionMamba(
        patch_size=16, embed_dim=128, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
...
def local_vim_tiny(pretrained=False, **kwargs):
    directions = (
        ...
        # ['h', 'h_flip', 'w2_flip', 'w7'],
        # ['v', 'v_flip', 'w2', 'w2_flip'],
        # ['h', 'h_flip', 'v', 'v_flip'], added
    )
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=10, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean',
        if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", directions=directions,
        mamba_cls=MultiMamba, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="to.do",
...
def local_vim_tiny_middle_cls_token(pretrained=False, **kwargs):
    ...
    model = VisionMamba(
        patch_size=16, embed_dim=128, depth=9, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean',
        if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", directions=directions,
        mamba_cls=MultiMamba,
...
def local_vim_small(pretrained=False, **kwargs):
    ...
    model = VisionMamba(
        patch_size=16, embed_dim=384, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
...
def local_vim_tiny_wo_search(pretrained=False, **kwargs):
    ...
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=20, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean',
        if_abs_pos_embed=True, if_rope=True, if_rope_residual=True, bimamba_type="v2", directions=directions,
        mamba_cls=MultiMamba, **kwargs)
    model.default_cfg = _cfg()
